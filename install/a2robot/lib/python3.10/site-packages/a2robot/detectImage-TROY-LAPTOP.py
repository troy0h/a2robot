...
class DetectImage(Node):
    ...
    def camera_image_ai(self, msg):
        ...
        if class_name == "1 left":
            self.publisher.publish("on left")
            self.publisher.publish("go left")
            self.get_logger().info('Publishing left')

        if class_name == "2 right":
            self.publisher.publish("on right")
            self.publisher.publish("go right")    
            self.get_logger().info('Publishing right')

        elif class_name == "3 stop":
